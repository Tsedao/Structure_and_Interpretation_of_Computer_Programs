...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        vals_list = []
        while formals != (nil):
            formals_list.append(formals.first)
            formals = formals.second
        while vals != (nil):
            vals_list.append(vals.first)
            vals = vals.second
        if len(formals_list) != len(vals_list):
            raise SchemeError
        for i in range(len(formals_list)):
            child.bindings[formals_list[i]] = vals_list[i]
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** REPLACE THIS LINE ***"
        new_expressions = Pair(target.second, expressions.second)
        env.define(target.first, do_lambda_form(new_expressions, env))
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings != (nil):
        first_bindings = bindings.first
        check_form(first_bindings, 2, 2)
        symbol = Pair(first_bindings.first, symbol)
        value = Pair(scheme_eval(first_bindings.second.first, env), value)
        bindings = bindings.second
    check_formals(symbol)
    return env.make_child_frame(symbol, value)
    "*** REPLACE THIS LINE ***"
    # END PROBLEM 15
